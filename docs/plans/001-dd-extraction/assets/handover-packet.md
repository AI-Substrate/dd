...
**Reproduced verbatim as of `1148efb`** — 15 rows, the commit that
...
14. DERIVE THE COMMIT MESSAGE, DO NOT COMPOSE IT (plan 001 P-1, raised by the PM after the o-prime's third instance). Write the commit message body from `git diff --cached`, not from what you intended to stage. The o-prime produced this class three times — a task title claiming a freeze that was held, a stale first-push premise, and a message that omitted 212 lines of another agent's work it actually carried. All three are the same shape as the stamped-but-unrun figure: RECORDED, NOT RUN. Filed as its own row rather than a clause of the measured-at rule because the ACTION differs — that rule says stamp or derive a measurement; this one says read what you are about to commit before you describe it.

15. A DECAYING FIGURE CAN WEAKEN AN ARGUMENT, NOT JUST AN HONESTY CLAIM (plan 001 P-3, found by the phase-4 coder re-sweeping on the could-this-rot test after rejecting its own first pass). The handover packet's STRONGEST claim was resting on a decaying number: 'library consumption is forced' was carried by a count of type-only symbols. The claim never needed the count — the argument is structural — so it had inherited that number's decay for nothing. THE FIX IS NOT TO STAMP THE FIGURE; IT IS TO NOTICE THE ARGUMENT NEVER NEEDED IT. When a figure appears inside an argument, ask what the argument would lose without it: if nothing, cut it; stamp it only where it is genuinely load-bearing. COROLLARY: decoupling can also reveal a claim leaning on its WEAKER support — here the type-erasure argument is rebuttable ('so redeclare the types'), while the load-bearing support is that two loaders are INJECTED INTO dd's walks and two helpers are called INLINE per table cell. Ask which support is actually carrying the claim. SECOND CORRECTION: strengthening an argument is itself a place claims outrun evidence, and the direction of the error is predictable — you overshoot toward the MODAL claim (impossible, always, never) because it reads stronger. A claim about SUBSTITUTION is provable; a claim about POSSIBILITY hands your reader a technicality that discredits a correct recommendation. Prefer the form that puts a COST in front of the decision-maker over the form that puts a BARRIER there.
```
...
**Reproduced verbatim as of `0a39125`** — 10 constraints,
