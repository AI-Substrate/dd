...
| F-06 | Docs are **generated**: `docs/how/dd/*.md` + `src/docs/dd-docs-manifest.json` → `src/docs/docs-content.ts`, drift-guarded | `scripts/gen-dd-docs.mjs:24`, `scripts/check-dd-docs.mjs:6-10` | A docs task edits source markdown then regenerates; hand-editing `docs-content.ts` fails the gate | High |
| F-07 | The teaching site for shape keys is `how-to-add-a-schema` § 2 "Write `schema.json`", which already names `gate_terminal` per-key | `docs/how/dd/how-to-add-a-schema.md:23,73,175` | `tally` belongs beside `gate_terminal` in § 2 + the Checklist — an existing shape, not a new chapter | High |
...
- **Likely files/symbols**: `docs/how/dd/how-to-add-a-schema.md` § 2 + Checklist and `deterministic-documents.md` (F-07/F-08); `src/render/renderer.ts` footer labelling (F-10); `src/core/tally.ts` for the empty-row and precision semantics; `src/acts/schema.ts` only if Q1 rules write.
