 ...
 * silently move `#tasks` and break every inbound link, with `ddocs doctor`
...
function renderObjectRows(
  ...
  // makes `ddocs build --check` a drift gate over the tally too: the markdown is a
  // faithful function of the JSON, and any disagreement between the JSON and its
  // own rows is `ddocs validate`'s finding to report, not something the renderer
 ...
 * `ddocs build --check` a usable drift gate.
 *
