...
const SURFACE = [
  {
    ...
    manifestNeedle: '`ddocs validate <path> [--depth <n>] [--json]`',
  },
  {
    file: 'schema',
    codeNeedles: ["command('list')"],
    manifestNeedle: '`ddocs schema list [--json]`',
  },
  {
    file: 'schema',
    codeNeedles: ["command('show <name>')"],
    manifestNeedle: '`ddocs schema show <name> [--json]`',
  },
  {
    file: 'docs',
    codeNeedles: ["command('list')"],
    manifestNeedle: '`ddocs docs list [--json]`',
  },
  {
    file: 'docs',
    codeNeedles: ["command('get <id>')"],
    manifestNeedle: '`ddocs docs get <id> [--json]`',
  },
  {
    file: 'build',
    codeNeedles: ["command('build <path>')", ".option('--check'"],
    manifestNeedle: '`ddocs build <path> [--check] [--json]`',
  },
  {
    file: 'address',
    codeNeedles: ["command('generate <interior>')", ".option('--path <path>'"],
    manifestNeedle: '`ddocs address generate <interior> [--path <path>] [--json]`',
  },
  {
    file: 'address',
    codeNeedles: ["command('validate <address>')", ".option('--resolve'"],
    manifestNeedle: '`ddocs address validate <address> [--resolve] [--json]`',
  },
  {
    file: 'link',
    codeNeedles: ["command('resolve <address>')"],
    manifestNeedle: '`ddocs link resolve <address> [--json]`',
  },
  {
    file: 'link',
    codeNeedles: ["command('verify-basis <address>')", ".requiredOption('--sha <sha>'"],
    manifestNeedle: '`ddocs link verify-basis <address> --sha <sha> [--json]`',
  },
  {
    file: 'links',
    codeNeedles: ["command('links <target>')"],
    manifestNeedle: '`ddocs links <target> [--json]`',
  },
  {
    file: 'graph',
    codeNeedles: ["command('graph')"],
    manifestNeedle: '`ddocs graph [--json]`',
  },
  {
    file: 'doctor',
    codeNeedles: ["command('doctor')"],
    manifestNeedle: '`ddocs doctor [--json]`',
  },
  {
    file: 'write',
    codeNeedles: ["command('get <address>')"],
    manifestNeedle: '`ddocs get <address> [--json]`',
  },
  {
    file: 'write',
    codeNeedles: ["command('set <address> <value>')", ".option('--value-json'"],
    manifestNeedle: '`ddocs set <address> <value> [--value-json] [--json]`',
  },
  {
    file: 'write',
    codeNeedles: ["command('add <address> <json>')", ".option('--mint <prefix>'"],
    manifestNeedle: '`ddocs add <address> <json> [--mint <prefix>] [--json]`',
  },
  {
    file: 'write',
    codeNeedles: ["command('rm <address>')"],
    manifestNeedle: '`ddocs rm <address> [--json]`',
  },
] as const;
...
describe('dd frozen surface manifest', () => {
  ...
  it('records the one-way extension reservations', () => {
    expect(MANIFEST).toContain('`ddocs doctor` scope/options');
    expect(MANIFEST).toContain('`ddocs graph` emit/scope options');
    expect(MANIFEST).toContain(
      '`ddocs link verify-basis` explicit re-verification mutation semantics',
    );
    expect(MANIFEST).toContain('`ddocs address validate --resolve` segment classification');
  });
});
